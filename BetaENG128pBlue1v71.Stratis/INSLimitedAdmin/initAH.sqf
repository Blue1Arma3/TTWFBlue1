...
playermessage = "INSLimitedAdmin: " + _player + " Has Been Caught Hacking, Contact An Admin Now! UID:" + _uid;
...
while {true} do
{
waituntil{((_bannedwepsdetect) && ((currentWeapon player) in _bannedweps)) or ((_bannedvclsdetect) && ((TypeOf (vehicle player)) in _bannedvcls)) or (broadcast == 1) or ((_speedhackdetect) && (player == vehicle player) && (round speed (vehicle player) > 30) && (alive player)) or ((_speedhackdetect) && (player != vehicle player) && (round speed (vehicle player) > _maxspeed))};
if ((_speedhackdetect) && !((vehicle player) in vehicles) && (round speed (vehicle player) > 30) && (alive player) && ((getposATL player) select 2 < 2)) then
	{
	sleep 2;
