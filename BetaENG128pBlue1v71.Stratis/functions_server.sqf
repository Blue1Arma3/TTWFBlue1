...
Tee_Server_Restart = {
	...
	private[_center,_phqw,_phqe]; 
	_phqw = getPosATL hq_west;	_phqe = getPosATL hq_east;
	_center = [0,0,0];
	_center set [0,((_phqw select 0)+(_phqe select 0))/2];
	_center set [1,((_phqw select 1)+(_phqe select 1))/2];
	_center set [2,((_phqw select 2)+(_phqe select 2))/2];

	//Repair Everything
	{
		_x setDamage 0;
	} forEach (_center nearObjects 4000);
...
Tee_Server_CleanUp = {
	...
	//Save Shop Locations
	{
		_shoploc set [count _shoploc, [_x,getPosATL _x]];
	} forEach _shops;
...
*/		
		...
		if(_i >= 10) then {
			...
			//Check Shops Posiion
			{
				if((_x select 0) distance (_x select 1) > 1) then {
