...
if ((isDedicated) || ! Team_Member) exitWith {};
...
_mainMenu = {
  ...
  if !isNil("_loadout") then {
    ...
    for "_i" from 0 to count(_loadout) do {
      ...
      if !isNil("_l") then {
        _any = true;    
        _actions = _actions + [_obj addAction [format["<t color='#00cc00'>Load </t><t color='#ffffff' size='1.5'>%1</t>",_l select 0], "loadout\loadout_manager.sqf", ["load",_i], 3000-_i]];
      };
    };
    if _any then {
      _actions = _actions + [ _obj addAction ["<t color='#ff1111'>Remove loadout ...</t>", "loadout\loadout_manager.sqf", ["remove_menu"], 2004] ];
    };
  };
  _actions = _actions + [ _obj addAction ["<t color='#ff8822'>Save loadout ...</t>", "loadout\loadout_manager.sqf", ["save_menu"], 2008] ];
  _actions = _actions + [ _obj addAction ["<t color='#0099ee'>Load VAS loadout ...</t>", "loadout\loadout_manager.sqf", ["vas_menu"], 2000] ];
};
...
switch (_args select 0) do {
  ...
  case "save_menu": {
    ...
    call _removeActions;    
    _actions = _actions + [_obj addAction ["<t color='#cccccc'>... back</t>", "loadout\loadout_manager.sqf", ["back"], 3001, false, false]];
    _actions = _actions + [_obj addAction ["<t color='#ffcc66'>Save as new</t>", "loadout\loadout_manager.sqf", ["save",-1], 3000]];
    
    for "_i" from 0 to (count(_loadout)-1) do {
      _l = _loadout select _i;
      if !isNil("_l") then {    
        _actions = _actions + [_obj addAction [format["<t color='#ff8822'>Replace </t><t color='#ffffff' size='1.5'>%1</t>",_l select 0], "loadout\loadout_manager.sqf", ["save",_i], 2000-_i]];
      };
    };
  ...
  case "remove_menu": {
    ...
    call _removeActions;    
    _actions = _actions + [_obj addAction ["<t color='#cccccc'>... back</t>", "loadout\loadout_manager.sqf", ["back"], 3000, false, false]];
      
    for "_i" from 0 to (count(_loadout)-1) do {
      _l = _loadout select _i;
      if !isNil("_l") then {    
        _actions = _actions + [_obj addAction [format["<t color='#ff1111'>Remove </t><t color='#ffffff' size='1.5'>%1</t>",_l select 0], "loadout\loadout_manager.sqf", ["remove",_i], 2000-_i]];
      };
    };
  ...
  case "remove": {
    ...
    hint parseText format["<t size='1' color='#ff1111'>Removed loadout</t>"];
    ...
    _actions = _actions + [_obj addAction ["<t color='#cccccc'>... back</t>", "loadout\loadout_manager.sqf", ["back"], 3000, false, false]];
		...
		for "_i" from 0 to 9 do {
			if(!isnil {profileNameSpace getVariable format["vas_gear_new_%1",_i]}) then {
				...
				_actions = _actions + [_obj addAction [format["<t color='#0099ee'>Load %1</t>", _loadout select 0], "loadout\loadout_manager.sqf", ["vas_load_new",_i], 2000-_i]];
			};
		};
		...
		for "_i" from 0 to 9 do {
			if(!isnil {profileNameSpace getVariable format["vas_gear_%1",_i]}) then {
				...
				_actions = _actions + [_obj addAction [format["<t color='#0088ee'>Load %1</t>", _loadout select 0], "loadout\loadout_manager.sqf", ["vas_load",_i], 1000-_i]];
			};
		};
...
4 _magazines ["16Rnd_9x21_Mag","HandGrenade","HandGrenade","SmokeShellBlue","SmokeShellBlue","HandGrenade","SmokeShellBlue","SLAMDirectionalMine_Wire_Mag","SLAMDirectionalMine_Wire_Mag","SLAMDirectionalMine_Wire_Mag","HandGrenade","HandGrenade","HandGrenade","HandGrenade","HandGrenade","SmokeShellBlue","SmokeShellBlue","SmokeShellBlue","SmokeShellBlue","SmokeShellBlue","DemoCharge_Remote_Mag","DemoCharge_Remote_Mag","SLAMDirectionalMine_Wire_Mag","SatchelCharge_Remote_Mag","DemoCharge_Remote_Mag","SLAMDirectionalMine_Wire_Mag","SLAMDirectionalMine_Wire_Mag","HandGrenade","HandGrenade","HandGrenade","HandGrenade","HandGrenade","HandGrenade"],
