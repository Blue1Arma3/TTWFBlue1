...
BTC_fnc_tk_PVEH =
{
	...
	//titleText ["has committed TK and has been punished by %2",_name,_punisher"PLAIN DOWN"];
...
BTC_EH_killed =
{
	...
	if (_name != name _body && BTC_vip find (name _killer) == -1) then
	{
		...
		{
			//BTC_tk_PVEH = [_name];publicVariable "BTC_tk_PVEH";player sidechat format ["%1 as commis un teamkill",_name];
			_killer spawn 
			{
				//titletext ["%1 teamkilled you! You can decide to punish him by action menu", name _this];
				hint format ["%1 teamkilled you! You can decide to punish him by action menu!", name _this];
				WaitUntil {Alive player};
				_action = player addAction [("<t color=""#ED2744"">") + ("Punish for TK" + name _this) + "</t>","=BTC=_TK_punishment\=BTC=_punish_action.sqf",[name _this], 8, true, true, "", "true"];
				_timeout = time + 90;
...
BTC_Teamkill =
{
	...
	switch (true) do
	{
		case (BTC_teamkiller <= BTC_tk_blackscreen_punishment) : 
		{
			...
			titleText ["STOP TEAMKILL!","BLACK FADED"];
			sleep 10;
			titleText ["STOP TEAMKILL!","PLAIN"];
		};
		...
		case (BTC_teamkiller > BTC_tk_last_warning) : 
		{
			...
			disableUserInput true;
			titleText ["YOU HAVE BEEN PUNISHED FOR TEAMKILLING!","BLACK FADED"];
			player setPos [0,0,0];
			while {true} do
			{
				titleText ["YOU HAVE BEEN PUNISHED FOR TEAMKILLING!","BLACK FADED"];
